...
def parse_all_xmls():
    ...
    for path in xml_files:
        ...
        for stmt in stmts:
            ...
            for opt in stmt.findall('.//OptionEAE'):
                ...
                option_eae[key] = {**opt.attrib, 'stmtDate': stmt_date}
    ...
    return trades, transfers, cash_txns, navs, option_eae, positions_map

print("正在解析 XML 文件...")
trades, transfers, cash_txns, navs, option_eae, positions_map = parse_all_xmls()
positions_list = list(positions_map.values())
print(f"解析完成: {len(trades)} 笔交易, {len(cash_txns)} 笔现金流, {len(navs)} 天净值, {len(option_eae)} 笔期权行权")
...
for k, v in trades.items():
    if v.get('accountId') == target_account and v.get('assetCategory') in ('STK', 'ETF'):
        ...
        if assignments and qty > 0:
            # 找到最匹配的一笔 assignment（按股数最接近）
            best_match = min(assignments, key=lambda a: abs(a['shares'] - qty))
            if abs(best_match['shares'] - qty) < 1:  # 完全匹配
                put_call = best_match['put_call']
                premium = best_match['premium']
                
                # Short Put Assignment (买入股票): 成本 = 行权价*股数 - 权利金
                # Long Call Assignment (买入股票): 成本 = 行权价*股数 + 权利金 (premium 通常为负)
                if bs == 'BUY':
                    if put_call == 'P':  # Put Assignment -> 通常 Short Put，成本减去权利金
                        premium_adjustment = -premium
                        note = f"opt_assign_put({best_match['symbol']})"
                    elif put_call == 'C':  # Call Assignment -> Long Call，成本加上权利金
                        premium_adjustment = premium  # premium 为负，所以加上相当于减去绝对值？不对
                        # mtm_pnl 对 Long Call 可能是负值（权利金支出），所以成本 = trade_money + |premium| = trade_money - premium（因为 premium < 0）
                        # 实际上 trade_money 已经是 strike * shares，对于 Long Call Assignment，IBKR 可能会单独计算
                        # 但这里保险起见，如果 premium < 0（支付），成本应该增加
                        premium_adjustment = -premium if premium < 0 else premium
                        note = f"opt_assign_call({best_match['symbol']})"
                
                # Short Call Assignment (卖出股票): 收入 = 行权价*股数 + 权利金
                # Long Put Assignment (卖出股票): 收入 = 行权价*股数 - 权利金
                elif bs == 'SELL':
                    if put_call == 'C':  # Call Assignment -> Short Call，收入加上权利金
                        premium_adjustment = premium
                        note = f"opt_assign_call({best_match['symbol']})"
                    elif put_call == 'P':  # Put Assignment -> Long Put，收入减去权利金
                        premium_adjustment = -premium if premium < 0 else premium
                        note = f"opt_assign_put({best_match['symbol']})"
                
                # 移除已使用的 assignment，避免重复匹配
...
for k, v in option_eae.items():
    if v.get('accountId') != target_account:
        continue
    date = v.get('date', '')
    sym = v.get('underlyingSymbol', '')
    opt_sym = v.get('symbol', '')
    pc = v.get('putCall', '')
    qty_c = float(v.get('quantity', 0) or 0)
    shares = qty_c * 100
    premium = float(v.get('mtmPnl', 0) or 0)
    strike = float(v.get('strike', 0) or 0)
    cost_before = strike * shares
    if pc == 'P':
        cost_after = cost_before - premium
        action = '买入'
    else:
        cost_after = cost_before + premium
        action = '买入/卖出'
    print(f"  {date} | {opt_sym} | {action} {shares:.0f}股 {sym} | 原成本={cost_before:,.2f} | 权利金={premium:,.2f} | 调整后={cost_after:,.2f}")

csv_path = '/Users/mc/kimicode/ib_dashboard/cost_basis_report_v4.csv'
