                    ...
                    <tr key={`${a.user_id}-${a.account_id}`} className="border-b border-[var(--lighter-gray)]">
                      ...
                      <td className="py-2">
                        <div className="flex flex-wrap gap-1">
                          ...
                          <a href={`/${a.account_id}/overview?admin_preview_user=${a.user_id}`} target="_blank" className="inline-block rounded border px-2 py-1 text-xs hover:bg-black hover:text-white">Dashboard</a>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {statsAccount && accountStats && (
        <Modal onClose={() => { setStatsAccount(null); setAccountStats(null); }} title={`账户统计: ${statsAccount.account_id}`}>
          <div className="space-y-4 text-sm">
            ...
            <div className="flex flex-wrap gap-2">
              <a href={`/${statsAccount.account_id}/overview?admin_preview_user=${statsAccount.user_id}`} target="_blank" className="inline-block rounded-lg bg-black px-4 py-2 text-sm text-white hover:bg-gray-800">查看 Dashboard</a>
