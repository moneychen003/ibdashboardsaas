...
def parse_all_xmls():
    ...
    return trades, transfers, cash_txns, navs, positions_list

print("正在解析 XML 文件...")
trades, transfers, cash_txns, navs, positions_list = parse_all_xmls()
print(f"解析完成: {len(trades)} 笔交易, {len(cash_txns)} 笔现金流, {len(navs)} 天净值")
...
print("\n外部现金流汇总（折算为 CNY，基础货币）:")
for acc, amt in sorted(cf_by_account.items()):
    print(f"  {acc}: {amt:>15,.2f} CNY")
total_cf = sum(cf_by_account.values())
print(f"  {'合计':<10}: {total_cf:>15,.2f} CNY")
...
for (acc, d), v in sorted(latest_navs.items()):
    print(f"  {acc}: {v['ending_value']:>15,.2f} CNY")
    total_market += v['ending_value']
print(f"  {'合计':<10}: {total_market:>15,.2f} CNY")
...
# 只计算 U12672188 的持仓（因为 U11181997 已基本清空）
...
# 3c. 按 symbol 计算摊薄成本
cost_records.sort(key=lambda x: x['date'])
cost_basis_by_symbol = {}

for symbol, group in defaultdict(list, {r['symbol']: [] for r in cost_records}).items():
    for r in cost_records:
        if r['symbol'] == symbol:
            group.append(r)

results = []
for symbol, records in cost_basis_by_symbol.items():
    total_qty = 0.0
    avg_cost = 0.0
    for r in records:
        qty = r['qty']
...
# 3d. 匹配最新持仓
latest_pos = [p for p in positions_list if p['account_id'] == target_account and p['date'] == latest_nav_date]
latest_pos.sort(key=lambda x: x['position_value'], reverse=True)

csv_rows = []
for p in latest_pos:
    sym = p['symbol']
    cb = cost_basis_by_symbol.get(sym, {'qty': 0, 'avg_cost': 0, 'total_cost': 0})
    if cb['qty'] <= 0.0001:
        continue
    
    # 持仓量以计算值为准（XML OpenPosition 的 position 有时为0但 position_value 有值）
    qty = cb['qty']
    ...
    csv_rows.append({
        ...
        '名称': p['description'][:30],
        '持仓量': round(qty, 4),
        '现价': round(p['mark_price'], 2),
        '市值': round(market_value, 2),
        '摊薄成本': round(cb['avg_cost'], 4),
        '总成本': round(cb['total_cost'], 2),
        '未实现盈亏': round(unrealized, 2),
        '盈亏比例': f"{return_pct:.2%}",
    })

print(f"\n当前持仓 ({target_account}, 日期: {latest_nav_date}):\n")
print(f"{'Symbol':<10} {'持仓量':>10} {'现价':>10} {'市值':>14} {'摊薄成本':>10} {'总成本':>14} {'未实现盈亏':>14} {'盈亏':>8}")
print("-" * 100)
total_mv = 0
total_cost = 0
for row in csv_rows:
    print(f"{row['Symbol']:<10} {row['持仓量']:>10.2f} {row['现价']:>10.2f} {row['市值']:>14,.2f} {row['摊薄成本']:>10.4f} {row['总成本']:>14,.2f} {row['未实现盈亏']:>14,.2f} {row['盈亏比例']:>8}")
    total_mv += row['市值']
    total_cost += row['总成本']
print("-" * 100)
...
with open(csv_path, 'w', newline='', encoding='utf-8-sig') as f:
    writer = csv.DictWriter(f, fieldnames=['Symbol', '名称', '持仓量', '现价', '市值', '摊薄成本', '总成本', '未实现盈亏', '盈亏比例'])
    writer.writeheader()
