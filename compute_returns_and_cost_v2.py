...
total_cost = 0
for row in csv_rows:
    print(f"{row['Symbol']:<10} {row['持仓量']:>10.2f} {row['现价']:>10.2f} {row['市值']:>14,.2f} {row['摊薄成本']:>10.4f} {row['总成本']:>14,.2f} {row['未实现盈亏']:>14,.2f} {row['盈亏比例']:>8.2%}")
    total_mv += row['市值']
    total_cost += row['总成本']
print("-" * 100)
...
with open(csv_path, 'w', newline='', encoding='utf-8-sig') as f:
    fieldnames = ['Symbol', '名称', '持仓量', '现价', '市值', '摊薄成本', '总成本', '未实现盈亏', '盈亏比例']
